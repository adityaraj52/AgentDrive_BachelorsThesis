...
{  
	...
	@Action(
			...
			defaultDelay = 0,
			defaultRepeat = 0,
			param = {@Parameter(name="trafficObjectID", type="Integer", defaultValue="0",
	...
	public TriggerAction moveTraffic(SimulationBasics sim, float delay, int repeat, Properties parameterList)
	{
		...
		try {
			...
			String property = parameterList.getProperty(parameter);
			if(property == null)
				throw new Exception();
            int trafficObjectID = Integer.parseInt(property);
	...
	{	
		...
		try {
			...
			// create ReportSpeedTriggerAction
			return new ReportTrafficLightTriggerAction(delay, repeat, trafficLightID, trafficLightState, (Simulator)sim);
