...
public class ScenarioLoader 
{
	...
	private void processSceneCar() 
	{
        ...
        driverCarName = dtData.getValue(Layer.SCENARIO,
                "/scenario:scenario/scenario:driver/scenario:car/@id", String.class);
	...
	private void extractRoadInformation() 
	{
		...
		try {
			...
			for (int k = 1; k <= laneNodes.getLength(); k++) 
			{
				...
				{
					LaneLimit laneLimit = new LaneLimit(Integer.parseInt(laneID), xMin, xMax);
    ...
    public String getDriverCarName() { return driverCarName; }
		...
		try {
			...
			for (int k = 1; k <= pointNodes.getLength(); k++) 
			{
				...
				*/
				...
				Integer id = dtData.getValue(Layer.SCENARIO,
						"/scenario:scenario/scenario:traffic/scenario:vehicle["+k+"]/@id", Integer.class);
                ...
                System.out.println(""+id+mass+acceleration+decelerationBrake+decelerationFreeWheel+engineOn+modelPath+curveTension+maxDistance
                +pathIsCycle+pathIsVisible+startWayPoint);
                System.out.println("creating trafic data "+k);
                TrafficCarData trafficCarData = new TrafficCarData(id, mass, acceleration, decelerationBrake,
                ...
                System.out.println("adding to map trafic data "+k);

                PhysicalTraffic.getVehicleDataMap().put(id, trafficCarData);
			}
