	...
	private void generateTrafficLightTriggers() 
	{
		for(Spatial object : sceneNode.getChildren())
		{
			...
			if (objectName.startsWith("TrafficLight")) 
			{
				...
				Box trigger1 = new Box(0.4f, 0.2f, 12f);
				Spatial triggerBox1 = new Geometry("TrafficLightTriggerGeometry:" + objectName, trigger1);
				...
				triggerBox1.setLocalTranslation(relativePos1);
				...
				Node triggerNode1 = new Node("TrafficLightTrigger:" + objectName);
				triggerNode1.attachChild(triggerBox1);
				
				// rotate trigger in the same way as the traffic light in origin
				triggerNode1.setLocalRotation(localRotation);
				
				// move rotated trigger from origin to a location next to the traffic light
				triggerNode1.setLocalTranslation(localTranslation);
				
				// attach trigger to trigger list
				triggerList.add(triggerNode1);		
				...
				// create trigger in origin
				Box trigger2 = new Box(0.4f, 0.2f, 50f);
				Spatial triggerBox2 = new Geometry("TrafficLightPhaseTriggerGeometry:" + objectName, trigger2);
				...
				triggerBox2.setLocalTranslation(relativePos2);
				...
				Node triggerNode2 = new Node("TrafficLightPhaseTrigger:" + objectName);
				triggerNode2.attachChild(triggerBox2);
				
				// rotate trigger in the same way as the traffic light in origin
				triggerNode2.setLocalRotation(localRotation);
				
				// move rotated trigger from origin to a location next to the traffic light
				triggerNode2.setLocalTranslation(localTranslation);
				
				// attach trigger to trigger list
				triggerList.add(triggerNode2);
			}
		}
	}
