...
import cz.agents.alite.protobuf.ProtobufClient;
...
import cz.agents.alite.protobuf.DLRProtobufClient;
...
import eu.opends.taskDescription.LaneKeepingTask;
...
public class Simulator extends SimulationBasics {
    ...
    public static String driverName = "car0";
    ...
    public void setSteeringCar(SteeringCar car) {
        this.car = car;
        physicalTraffic.setEgoCar(car);
    }
    ...
    private static ProtobufClient protoClient;

    public ProtobufClient getProtobufClient() {
        return protoClient;
    }

    private static LaneAgent laneAgent;

    public LaneAgent getLaneAgent() {
        return laneAgent;
    }
    ...
    private LaneKeepingTask laneKeepingTask;

    public LaneKeepingTask getLaneKeepingTask() {
        return laneKeepingTask;
    }
    ...
    public void simpleInitApp() {
        showStats(false);
        PropertyConfigurator.configure("assets/DrivingTasks/Projects/Highway/log4j/log4j.properties");
        if (drivingTaskGiven)
    ...
    public void simpleInitDrivingTask(String drivingTaskFileName, String driverName) {
        ...
        laneAgent = new LaneAgent(this);

        protoClient = new ProtobufClient(this, "socket://localhost:2222");

        // create and place steering car
        //car = new SteeringCar(this);
        //System.out.println("Streering car posi:"+car.getPosition());

        // initialize physical vehicles
        physicalTraffic = new PhysicalTraffic(this);
        // AgentDrive determines which vehicles to simulate
        protoClient.init();

       // physicalTraffic.start(); //TODO

        // sync driver name with KAPcom. May provide suggestion for driver name if
        // driverName = KnowledgeBase.User().initUserName(driverName);

        int attempt=5;
        while (this.car==null && attempt-- >0){
            System.out.println("Waiting for communicator to receive initial plan...");
            try {
                System.out.println("Waiting for communicator to receive initial plan...");
        ...
        drivingTaskLogger = new DrivingTaskLogger(outputFolder, driverName,
                drivingTask.getFileName());
        ...
        laneKeepingTask = new LaneKeepingTask(this);
    ...
    /**
     * That method is going to be executed, when the dataWriter is <code>null</code> and the S-key
     * is pressed.
     * 
     */
    public void initializeDataWriter() {
        dataWriter = new DataWriter(outputFolder, car, driverName,
                SimulationDefaults.drivingTaskFileName);
    }
...
//            updateDataWriter();
            ...
            // send update via protobuf
            protoClient.update(tpf, this.timer.getTimeInSeconds());
    ...
    /**
     * Cleanup after game loop was left. Will be called when pressing any close-button. destroy()
     * will be called subsequently.
     */
    /*
     * @Override public void stop() { logger.info("started stop()"); super.stop();
     * logger.info("finished stop()"); }
     */

    /**
     * Cleanup after game loop was left Will be called whenever application is closed.
     */
    ...
    public static void main(String[] args) {
        try {
            ...
            PropertyConfigurator.configure("E://AgentDrive/simulator/assets/JasperReports/log4j/log4j.properties");

            /*
             * logger.debug("Sample debug message"); logger.info("Sample info message");
             * logger.warn("Sample warn message"); logger.error("Sample error message");
            ...
            settings.setSettingsDialogImage("Textures/Logo/OpenDS.png");
            settings.setTitle("OpenDS");

            // set splash screen parameters
            /*
             * settings.setFullscreen(false); settings.setResolution(1280, 720);
             * settings.setSamples(4); settings.setBitsPerPixel(24); settings.setVSync(false);
